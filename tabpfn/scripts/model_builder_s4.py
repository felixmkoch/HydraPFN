...
def load_model_only_inference(path, filename, device):
    ...
    mamba_num_layers = config_sample["s4_num_layers"]
...
def get_model_s4(config, device, should_train=True, verbose=False, state_dict=None, epoch_callback=None, s4_autocast=True, evaluation_class=None):
    ...
    from tabpfn.s4_train import train_s4
    extra_kwargs = {}
    ...
    model_mamba = train_s4(model_proto.DataLoader
                  , loss
                  , encoder
                  ...
                  , lr=config['lr']
                  , enable_autocast=s4_autocast
                  , num_mamba_layers=config['s4_num_layers']
