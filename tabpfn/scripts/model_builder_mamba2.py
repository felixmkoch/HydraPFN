...
def load_model_only_inference(path, filename, device):
    ...
    mamba_num_layers = config_sample["mamba2_num_layers"]

    attn_cfg = config_sample["attn_cfg"]
    attn_layer_idx = config_sample["attn_layer_idx"]
...
def get_model_mamba2(config, device, should_train=True, verbose=False, state_dict=None, epoch_callback=None, mamba_autocast=True, evaluation_class=None):
    ...
    from tabpfn.mamba2_train import train_mamba         # Mamba 2 here
    extra_kwargs = {}
    ...
    model_mamba = train_mamba(model_proto.DataLoader
                  ...
                  , nlayers=config['nlayers']           # Wrong ... this is the transformer layer parameter. DOesn't need to change, just deprecated.
                  ...
                  , num_mamba_layers=config['mamba2_num_layers']
