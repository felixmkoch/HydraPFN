...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True, model_path_custom=""):
    ...
    if only_inference:
        ...
        model, c = load_model_only_inference(base_path, model_file, device, model_name="hydra")
    else:
...
class HydraPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X, normalize_with_test=False, return_logits=False):
        ...
        prediction = hydra_predict(self.model[2], X_full, y_full, eval_pos,
                                         device=self.device,
                                         style=self.style,
                                         inference_mode=True,
...
import time
def hydra_predict(model, eval_xs, eval_ys, eval_position,
                        device='cpu',
                        max_features=100,
                        style=None,
