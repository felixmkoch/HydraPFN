...
class MambaPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X, normalize_with_test=False, return_logits=False):
        ...
        prediction = mamba_predict(self.model[2], X_full, y_full, eval_pos,
                                         device=self.device,
                                         style=self.style,
                                         inference_mode=True,
...
import time
def mamba_predict(model, eval_xs, eval_ys, eval_position,
                        device='cpu',
                        max_features=100,
                        style=None,
