...
def evaluate_tabarena(models,
    ...
    for i_dataset, task_id in enumerate(task_ids):
        try:

            if task_indices is not None and i_dataset not in task_indices:
                print(f"Skipping task {i_dataset} ({task_id}): not in selected "
                      "indices.")
                continue
            ...
            if task_type not in task_types:
                print(f"Skipping task {i_dataset} ({task_id}): wrong task type.")
                continue
            ...
            if max_samples is not None and n_samples > max_samples:
                print(f"Skipping task {i_dataset} ({task_id}): too many samples.")
                continue
            if max_features is not None and n_features > max_features:
                print(f"Skipping task {i_dataset} ({task_id}): too many features.")
                continue
            if max_classes is not None and n_classes > max_classes:
                print(f"Skipping task {i_dataset} ({task_id}): too many classes.")
                continue
            ...
            for repeat in range(n_repeats):
                for fold in range(n_folds):
                    print(f"====== Task {i_dataset}, repeat {repeat}, fold {fold}. "
                        "======")
            ...
            print(f"Error: {e}")
            print(f"Skipping task {i_dataset} ({task_id}): error.")
            continue
