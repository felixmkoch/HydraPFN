...
class HydraModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 device = "cpu",
                 dtype=None,
                 add_input_residual=False   # NEW FLAG
                 ) -> None:
        ...
        print(f"Add input residual embeddings: {add_input_residual}")
    ...
    def forward(
        ...
        **kwargs
        ...
        if P > 1:
            ...
            # Run backbone
            context_hidden_ssm = self.mamba_backbone(
                context_permuted,
                inference_parameters=None
            )

            # Add original embeddings if enabled
            if self.add_input_residual:
                context_hidden = context_hidden_ssm + context_permuted  # (P*B, S, D)
            else:
                context_hidden = context_hidden_ssm
        ...
        return output, context_hidden_ssm, perm_context_hidden
