...
def create_job_configs(main_config: om.DictConfig, tasks_to_run: Set[str], from_mnli=False):

    # Downloads the starting checkpoint ahead of time so that
    # the different tasks don't all try to download it at the same time
    if main_config.get('local_pretrain_checkpoint_folder', None) is None:
        pretrained_checkpoint_path = None
    else:
        if not from_mnli:
            if main_config.get('starting_checkpoint_load_path', None) is None:
                pretrained_checkpoint_path = os.path.join(
                    main_config.local_pretrain_checkpoint_folder, "latest-rank0.pt"
                )
            else:
                pretrained_checkpoint_path = download_starting_checkpoint(
                    main_config.starting_checkpoint_load_path,
                    main_config.local_pretrain_checkpoint_folder)
    ...
    for task_name, task_config in main_config.tasks.items():
        ...
        seeds = set()
        while True:
            seeds.add(random.randrange(1000000))
            if len(seeds) == main_config.num_task_run:
                break

        if from_mnli:
            mnli_path = os.path.join(main_config.local_pretrain_checkpoint_folder, "task=mnli")
            mnli_seeds = [int(d.replace("seed=", "")) for d in os.listdir(mnli_path) if os.path.isdir(os.path.join(mnli_path, d))]
            assert len(mnli_seeds) == len(seeds)

        for i, task_seed in enumerate(seeds):
            if from_mnli:
                pretrained_checkpoint_path = os.path.join(mnli_path, f"seed={mnli_seeds[i]}", "latest-rank0.pt")
                run_name = f'{main_config.base_run_name}_task={task_name}_mnli={str(mnli_seeds[i])}_seed={str(task_seed)}'
            else:
            ...
            task_seed_config = om.OmegaConf.create({
                ...
                'save_folder':
                    ...
                    ) if not from_mnli else os.path.join(
                        main_config.save_finetune_checkpoint_folder,
                        f'task={task_name}', f'mnli={mnli_seeds[i]}', f'seed={task_seed}'
                    ),
                'loggers':
                    logger_configs,
                'callbacks':
...
def train(config: om.DictConfig) -> None:
    ...
    config.default_seed = random.randrange(1000000)
    ...
    if config.from_mnli:
        ...
        from_mnli = False
        round_1_task_names = {'qqp', 'cola', 'sst2', 'qnli', 'mnli'}
    round_1_job_configs = create_job_configs(config, round_1_task_names, from_mnli=from_mnli)
